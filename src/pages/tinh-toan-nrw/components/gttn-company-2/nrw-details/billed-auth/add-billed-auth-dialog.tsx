...
interface AddBillAuthDialogProps {
    onSave?: (newRow: NrwCongTyTieuThuChiTietResponse) => void;
    onClose?: () => void;
    maDauVao?: number;
    nguoiTao?: number;
    ky: number;
...
const AddBillAuthDialog: React.FC<AddBillAuthDialogProps> = ({ onSave, onClose, maDauVao, ky, nam }) => {
    const { enqueueSnackbar } = useSnackbar();
    const initialMonth = new Date().getMonth() + 1;
    const { setMaDoiTuong } = useThemePanel();

    // State lưu dữ liệu form
    const [formData, setFormData] = useState<Partial<AddNrwCongTyTieuThuChiTietRequest>>({
        MaTieuThu: maDauVao,
        Ky: ky,
        Nam: nam,
        Nguon: "",
    ...
    const handleChange = (field: keyof AddNrwCongTyTieuThuChiTietRequest, value: string | number) => {
        setFormData((prev) => ({
            ...prev,
    ...
    const handleNumberChange = (field: keyof AddNrwCongTyTieuThuChiTietRequest, value: string) => {
    ...
    const handleDateChange = (field: keyof AddNrwCongTyTieuThuChiTietRequest, value: Date | null) => {
        setFormData((prev) => ({
            ...prev,
    ...
    const handleYearChange = (field: keyof AddNrwCongTyTieuThuChiTietRequest, value: Date | null) => {
        setFormData((prev) => ({
            ...prev,
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const newData = await createData<AddNrwCongTyTieuThuChiTietRequest, NrwCongTyTieuThuChiTietResponse>(
            ...
            <Box sx={{ p: 2 }}>
                <Grid container spacing={2}>
                    ...
                    {/* <Grid size={{ xs: 4 }}>
                        ...
                        <TextField
                            fullWidth
                            value={formData.MaTieuThu ?? ''}
                            onChange={(e) => handleChange('MaTieuThu', e.target.value)}
                            size="small"
                            required
                            disabled
                        />
...
export default AddBillAuthDialog;

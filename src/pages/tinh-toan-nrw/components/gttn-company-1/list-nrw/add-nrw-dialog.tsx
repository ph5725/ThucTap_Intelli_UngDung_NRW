...
// type
import { AddNrwCongTyRequest, NrwCongTyResponse, TongSanLuongRequest, TongSanLuongRespone, SanLuongTieuThuRequest, SanLuongTieuThuRespone, SoNgayDocSoBillingRequest, SoNgayDocSoBillingRespone } from 'src/types/nrw-cong-ty/nrw-cong-ty';
...
interface AddNrwDialogProps {
  onSave?: (newRow: NrwCongTyResponse) => void;
  onClose?: () => void;
  maDoiTuong?: number;
  nguoiTao?: number;
}
...
const AddNrwDialog: React.FC<AddNrwDialogProps> = ({ onSave, onClose, maDoiTuong = 0 }) => {
  ...
  const [formData, setFormData] = useState<Partial<AddNrwCongTyRequest>>({
    Ma: 'CongTy',
    Ky: initialMonth,
    Nam: new Date().getFullYear(),
    TuNgay: '',
    DenNgay: '',
    SoNgayDocSoDht: undefined,
    SoNgayDocSoBilling: undefined,
    NguyenNhan: '',
    GhiChu: '',
    SanLuongDauVao: 0,
    SanLuongTieuThu: 0,
  ...
  const handleChange = (field: keyof AddNrwCongTyRequest, value: string | number) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const handleNumberChange = (field: keyof AddNrwCongTyRequest, value: string) => {
  ...
  const handleDateChange = (field: keyof AddNrwCongTyRequest, value: Date | null) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const handleYearChange = (field: keyof AddNrwCongTyRequest, value: Date | null) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload: AddNrwCongTyRequest = {
        Ma: formData.Ma!,
        Ky: Number(formData.Ky)!,
        Nam: Number(formData.Nam)!,
        SanLuongDauVao: formData.SanLuongDauVao!,
        SanLuongTieuThu: formData.SanLuongTieuThu!,
        LuongNuocThatThoat: formData.LuongNuocThatThoat,
        TyLeThatThoatChuan1: formData.TyLeThatThoatChuan1,
        TyLeThatThoatChuan2: undefined,
        TuNgay: formData.TuNgay!,
        DenNgay: formData.DenNgay!,
        SoNgayDocSoDht: formData.SoNgayDocSoDht,
        SoNgayDocSoBilling: formData.SoNgayDocSoBilling,
      ...
      const newData = await createData<AddNrwCongTyRequest, NrwCongTyResponse>(
